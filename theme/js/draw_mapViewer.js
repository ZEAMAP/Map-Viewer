...
mapViewer = {  
   ...
   RectangleEnum: {CHR: 0, ZOOM: 1},
	...
	MarkerEvents: class {
		...
		onclick(node) {
			d3.select("#select_table_view_body").selectAll("tr").remove()
            d3.select("#select_table_view_body").selectAll("tr").data(node.nodes).enter().append("tr").html(function (d) {
				return '<td><a target="_blank" href="' + Drupal.settings.baseUrl + d.marker.url + '">' + d.fullName + '</a></td>'
            })
		}
	...
	MarkersOnRect: class {
      ...
      var markerPosTextAnchor = "end";
			...
			// console.log(_markerLineAllNodes);

			var _a_markerLineAllNodes = {}
			_markerLineAllNodes.forEach(function (d) {
				var key = d.x + "_" + d.y + "_" + d.y_init
				if (!_a_markerLineAllNodes.hasOwnProperty(key)) {
                    _a_markerLineAllNodes[key] = []
				}
                _a_markerLineAllNodes[key].push(d)
			})

			var _b_markerLineAllNodes = []
			for (var p in _a_markerLineAllNodes) {
				var xys = p.split("_")
				var _c_markerLineAllNode = {x: xys[0], y: xys[1], y_init: xys[2], nodes: _a_markerLineAllNodes[p]}
				_b_markerLineAllNodes.push(_c_markerLineAllNode)
			}
          ...
          .attr("font-size", "8")
	    	...
	    	if (this.type == mapViewer.RectangleEnum.ZOOM) {
    			...
    			marker_lines.selectAll("#MarkersOnRect line."+_this.marker_lines_className)
    				...
    				.on("click", function(d) {
    					d3.selectAll("#MarkersOnRect line."+_this.marker_lines_className).style("stroke", "#E59866")
    					d3.select(this).style("stroke", "#C11D00")
    					_this.markerEvents.onclick(d)
    				});
      ...
      // TODO remove all markers from this position
	...
	MapLabel: class {
		...
		//因为Map Detail的链接页面有问题，所以暂时去掉这个链接
		/*
